    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                RETURNING *
                """,
                (approved_by, now, now, expires_at, extension_id),
            )
            row = cur.fetchone()
    if not row:
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                RETURNING *
                """,
                (reason, now, extension_id, tenant_id),
            )
            row = cur.fetchone()
    if not row:
        raise ValueError("extension_not_found")
...
def clear_extension_expiry(extension_id: int, *, tenant_id: str) -> dict[str, Any]:
    now = datetime.now(timezone.utc)
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                RETURNING *
                """,
                (expires_at, now, extension_id, tenant_id),
            )
            row = cur.fetchone()
    if not row:
        raise ValueError("extension_not_found")
...
def list_learning_auto_extensions(*, tenant_id: str) -> list[dict[str, Any]]:
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
