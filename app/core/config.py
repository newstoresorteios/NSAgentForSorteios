...
class Settings(BaseSettings):
    ...
    agent_tool_policy_mode: Literal["off", "shadow", "enforce"] = Field(
        default="shadow",
        alias="AGENT_TOOL_POLICY_MODE",
    )
    ...
    # Phase-0 deterministic DoubleCheck. Shadow records; enforce only swaps PIX resume.
    agent_double_check_mode: Literal["off", "shadow", "enforce"] = Field(
        default="enforce",
        alias="AGENT_DOUBLE_CHECK_MODE",
    )
    ...
    @field_validator(
        "agent_critique_mode",
        "agent_double_check_mode",
        "agent_tool_policy_mode",
        mode="before",
    )
    @classmethod
