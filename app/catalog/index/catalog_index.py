...
def _infer_material(title: str, product: dict[str, Any]) -> str | None:
    from app.catalog.specs.catalog_specs import extract_material
...
def _hard_constraints_from_interpretation(
    ...
    try:
        from app.catalog.product_retrieval import effective_product_reference

        safe_reference = effective_product_reference(subject.reference)
    except Exception as exc:
        log_swallowed("index.hard_reference", exc)
    ...
    try:
        ...
        if turn is not None:
            ...
            try:
                ...
                log_swallowed("index.turn_reference", exc)
        ...
        log_swallowed("index.turn_hard_constraints", exc)
    return hard
    ...
    *,
    ...
    if hard.get("exact_only") or mode == "exact":
        ...
        if dial:
            ...
            try:
                from app.catalog.product_retrieval import expand_color_aliases

                aliases = expand_color_aliases(dial)
            except Exception as exc:
                log_swallowed("index.color_aliases", exc)
...
def score_soft_preferences(
    ...
    try:
        ...
        log_swallowed("index.soft_preferences", exc)
...
def upsert_canonical_items(items: list[CanonicalCatalogItem]) -> int:
    ...
    if not items:
        return 0
    try:
        from app.db import ensure_catalog_pg_trgm, ensure_tables, get_conn, to_jsonb
