...
def compile_agent_prompt(
    *,
    ...
    learned_cases_block = "<learned_cases>\n</learned_cases>"
    ...
    if load_persona_knowledge and active_persona is not None:
        try:
            from app.persona.persona_knowledge_repository import load_persona_knowledge_for_prompt
    ...
    if load_extensions:
        ...
        try:
            from app.learning.cases import format_learned_cases_block, list_active_cases
            ...
            learned_cases_block = format_learned_cases_block(
                learned_cases,
                max_chars=int(getattr(settings, "agent_max_learned_case_chars", 2000)),
            )
        except Exception as exc:
            print("[prompt.compiler.learned_cases.error]", {
                "error_type": type(exc).__name__,
                "error": str(exc)[:160],
            })
    ...
    # Layer order: see app.llm.prompt_layers.PROMPT_LAYER_ORDER (Etapa 7–8).
...
def _append_contact_memory_block(
    ...
    *,
    ...
    if inject and sender_key:
        try:
            from app.memory import contact_memory_repository as contact_memory_repository
