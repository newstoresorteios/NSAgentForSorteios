...
from app.catalog.media.product_media import resolve_product_image
...
async def test_inbound_photo_with_image_request_identifies_product(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.catalog.vision.image_product_id.handle_image_product_search",
        fake_image_search,
    )
    monkeypatch.setattr(
        "app.catalog.vision.image_product_id.image_search_eligible",
        lambda message: True,
    )
    monkeypatch.setattr(
        sales_agent,
