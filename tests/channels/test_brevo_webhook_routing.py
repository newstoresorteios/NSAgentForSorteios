...
@pytest.mark.asyncio
async def test_async_ingress_enqueues_normalized_payload(monkeypatch):
    import api.index as index
    ...
    monkeypatch.setattr(settings, "agent_async_ingress_enabled", True)
    monkeypatch.setattr("app.ingress.inbox.enqueue_inbound", fake_enqueue)
    monkeypatch.setattr(index, "inbound_message_exists", lambda *_args: False)
    monkeypatch.setattr(
        index,
        "claim_inbound_message",
        lambda _message: (_ for _ in ()).throw(
            AssertionError("async enqueue must not claim inbound")
        ),
    )
    ...
    normalized = captured["payload"]["normalized"]
    assert normalized["text"] == "mensagem inbound"
...
@pytest.mark.asyncio
async def test_sync_valor_followup_reattaches_recent_photo(monkeypatch):
    """Brevo sync path (async off) must bind 'valor' to the last inbound photo."""
    import api.index as index
    ...
    monkeypatch.setattr(
        index,
        "claim_inbound_message",
        lambda message: (claimed.append(message) or True, 404),
    )
    monkeypatch.setattr(index, "is_latest_inbound_message", lambda *_args: True)
    monkeypatch.setattr(index, "find_customer_profile_by_phone", lambda _phone: {})
    monkeypatch.setattr(index, "process_incoming_message", process)
    ...
    payload["messages"][0]["id"] = "message-valor"
    payload["messages"][0]["text"] = "valor"
    ...
    assert len(processed) == 1
    assert processed[0].image_url == "https://cdn.example.com/watch.jpg"
    assert processed[0].input_modality == "text_with_image"
    assert claimed[0]["image_url"] == "https://cdn.example.com/watch.jpg"
