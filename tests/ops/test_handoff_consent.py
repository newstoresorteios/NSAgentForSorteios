...
from app.ops.handoff_consent import customer_requests_human, consent_reason, DEFAULT_OFFER
...
def test_acceptance_requires_immediately_preceding_human_offer(text):
    offer = [{'role':'assistant','content': DEFAULT_OFFER}]
...
def test_ambiguous_or_negative_answer_is_not_consent(text):
    assert not is_handoff_acceptance(text, [{'role':'assistant','content':DEFAULT_OFFER}])
