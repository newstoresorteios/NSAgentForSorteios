...
def db(monkeypatch):
    ...
    db.executescript("""
        CREATE TABLE conversas(id TEXT PRIMARY KEY, workspace_id TEXT, channel TEXT,
          ...
          status TEXT, bot_activated BOOLEAN, assigned_to TEXT, updated_at TEXT);
...
def add(db, id, workspace="w1", channel="whatsapp", thread="thread", merged=None, status="active", assigned=None, canal="c1"):
    db.execute("INSERT INTO conversas VALUES (?,?,?,?,?,?,?,?,?,?,?)",
               (id, workspace, channel, canal, thread, "same-phone", merged, status, True, assigned, "before"))
...
def test_handoff_scoped_to_owner_channel_and_session_with_before_audit(db):
    ...
    assert handoff_queue.mark_conversa_for_human_handoff(incoming(raw={"inbound_id": 1}), reason="review") == ["target"]
    ...
    assert audit["_handoff"]["reason"] == "review"


def test_ambiguous_workspace_or_channel_account_never_updates(db):
    add(db, "one")
    add(db, "two", canal="another-account")
    assert handoff_queue.mark_conversa_for_human_handoff(incoming(), reason="review") == []
...
def test_missing_server_inbound_does_not_infer_owner_from_unique_thread(db):
    ...
    assert handoff_queue.mark_conversa_for_human_handoff(incoming(raw={}), reason="review") == []


def test_phone_without_conversation_never_updates(db):
    add(db, "target")
    assert handoff_queue.mark_conversa_for_human_handoff(IncomingMessage(channel="whatsapp", sender_phone="same-phone"), reason="review") == []


def test_missing_inbound_owner_never_falls_back_to_phone(db):
    add(db, "target")
    assert handoff_queue.mark_conversa_for_human_handoff(incoming(raw={"inbound_id": 99}), reason="review") == []
...
def test_merged_alias_resolves_only_same_workspace_canonical(db):
    ...
    assert handoff_queue.mark_conversa_for_human_handoff(incoming(), reason="review") == ["canonical"]
...
def test_existing_operator_and_closed_conversation_are_preserved(db):
    ...
    handoff_queue.mark_conversa_for_human_handoff(incoming(), reason="review")
