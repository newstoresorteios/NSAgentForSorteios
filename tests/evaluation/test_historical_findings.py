...
async def test_technical_match_with_missing_live_price_keeps_partial_evidence(price, upon_request):
    ...
    assert 'Modelo de teste' not in result.reply_text
    assert result.response_metadata['clear_active_product']
    assert not result.response_metadata.get('active_product')
