...
def test_compact_and_canonical_share_effective_price():
    from app.catalog.index.cache import compact_catalog_product
    from app.catalog.index.snapshot import product_dict_to_snapshot

    promo_only = {
        "id": "9",
        "name": "Casio MTP",
        "promotional_price": 199.9,
    }
    compact = compact_catalog_product(promo_only)
    item = to_canonical_item(promo_only)
    snap = product_dict_to_snapshot(promo_only)
    assert compact is not None and compact["price"] == 199.9
    assert item is not None and item.price == 199.9
    assert snap.price is not None and float(snap.price) == 199.9
    assert snap.promotional_price is not None and float(snap.promotional_price) == 199.9


def test_catalog_fold_is_shared():
    from app.catalog.category.resolver import _fold as category_fold
    from app.catalog.index.catalog_index import _fold as index_fold
    from app.catalog.retrieval.text import fold_text
    from app.catalog.specs.catalog_specs import _fold as specs_fold
    from app.catalog.specs.preference_normalize import _fold as pref_fold

    assert index_fold is fold_text
    assert specs_fold is fold_text
    assert pref_fold is fold_text
    assert fold_text("Seeländer") == "seelander"
    assert category_fold("Relógios") == "relogios"


def test_catalog_exception_handlers_are_not_bare_pass():
    import ast
    from pathlib import Path

    silent: list[str] = []
    for path in Path("app/catalog").rglob("*.py"):
