...
from app.catalog.category.resolver import CategoryResolver, CategorySelection, normalize_category_name
...
async def test_category_resolver_matches_real_plural_category(monkeypatch):
    ...
    monkeypatch.setattr("app.catalog.category.resolver.get_settings", lambda: _settings())
...
async def test_category_children_contribute_candidates_and_products_are_deduplicated(monkeypatch):
    ...
    monkeypatch.setattr("app.catalog.retrieval.rerank.rerank_products", rerank)
...
async def test_category_selector_discards_invented_id(monkeypatch):
    import app.catalog.category.resolver as resolver_module
